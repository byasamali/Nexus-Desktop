...
function MiniSepet({ cart }: { cart: Record<string, CartItem> }) {
              ...
              return (
                <div key={barkod} className="group px-3 py-2.5 hover:bg-stone-50/80 transition-colors">
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <button onClick={() => copyText(barkod, `barkod-${barkod}`)} title={`Barkod kopyala`}
...
function BrowserPanel({ tabs, activeTabId, onTabChange, onTabClose, onTabAdd, onNavigate }: {
  tabs: BrowserTab[];
  activeTabId: string;
      ...
      <div className="flex-1 relative bg-white">
        {tabs.map(tab => (
          React.createElement('webview', {
            key: tab.id,
            ref: (el: any) => { iframeRefs.current[tab.id] = el; },
